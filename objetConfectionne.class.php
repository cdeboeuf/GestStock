...
class ObjectConfectionne
{
    ...
    {
        $req="SELECT * FROM objetconfectionne INNER JOIN users on objetconfectionne.professeur=users.Id where NbRealise = '0';";
    ...
    {
        $req="SELECT *,date_format(objetconfectionne.DateEmi, '%d/%m/%Y')As DateEmiF,date_format(objetconfectionne.DateFabri, '%d/%m/%Y')As DateFabriF  FROM objetconfectionne INNER JOIN users on objetconfectionne.professeur=users.Id where Ref = '$ref';";
        $rs = ObjectConfectionne::$bdd->query($req);
        $result = $rs->fetchAll();
        return $result;
    }
        ...
        foreach ($idT as $id)
{
   $id=1+$id['ordre'];
    ...
    {
        $req="SELECT sum(ligneoc.quantite*ligneoc.PuTTC)As prix FROM ligneoc WHERE RefOc = '$Ref' group by RefOc";
        $rs=ObjectConfectionne::$bdd->query($req);
        return $result=$rs->fetchAll();
    }
